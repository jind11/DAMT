...
def check_data_params(params):
    ...
    if not params.para_data_path:
        params.para_data_path = params.data_path
    required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps)
    ...
    for src in params.langs:
        for tgt in params.langs:
            if src < tgt and ((src, tgt) in required_para or (tgt, src) in required_para):
                ...
                for splt in ['train', 'valid', 'test']:
                    if splt != 'train':
                        params.para_dataset[(src, tgt)][splt] = \
                            (os.path.join(params.data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, src)),
                            os.path.join(params.data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, tgt)))
                    else:
                        if (src, tgt) in required_para_train or (tgt, src) in required_para_train:
                            params.para_dataset[(src, tgt)][splt] = \
                                (os.path.join(params.para_data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, src)),
                                 os.path.join(params.para_data_path, '%s.%s-%s.%s.pth' % (splt, src, tgt, tgt)))
