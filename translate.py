...
def get_parser():
    ...
    parser.add_argument("--output_path_source", type=str, default="", help="Output path for source")
    parser.add_argument("--output_path_target", type=str, default="", help="Output path for target")
    ...
    parser.add_argument("--src_data_path", type=str, default="", help="Input data path")
...
def main(params):
    ...
    src_sent = []
    for line in open(params.src_data_path, 'r').readlines():
        if line.strip() and len(line.split()) <= 130:
            src_sent.append(line)
    logger.info("Read %i sentences from stdin. Translating ..." % len(src_sent))

    f_src = io.open(params.output_path_source, 'w', encoding='utf-8')
    f_tgt = io.open(params.output_path_target, 'w', encoding='utf-8')
    ...
    for i in range(0, len(src_sent), params.batch_size):
        ...
        try:
            ...
            print(max([len(line.split()) for line in src_sent[i:i + params.batch_size]]))
        else:
