...
epoch_size=$(cat /data/medg/misc/jindi/nlp/datasets/OPUS/$SRC-$TGT/$TGT_DATA_NAME/copy/$SRC_DATA_NAME/train.$SRC-$TGT.$SRC | wc -l)
...
python -W ignore train.py \
    --exp_name no_pretrain_sup_copy_src_$SRC_DATA_NAME\_tgt_$TGT_DATA_NAME\_$SRC\_$TGT \
    --dump_path ./tmp/ \
    --data_path /data/medg/misc/jindi/nlp/datasets/OPUS/$SRC-$TGT/$TGT_DATA_NAME/processed/$SRC-$TGT \
    --para_data_path /data/medg/misc/jindi/nlp/datasets/OPUS/$SRC-$TGT/$TGT_DATA_NAME/copy/$SRC_DATA_NAME \
    --lgs $SRC-$TGT \
    --mt_steps $SRC-$TGT \
    --encoder_only false \
    --emb_dim 1024 \
