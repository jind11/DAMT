...
PROC_PATH=$DATA_PATH/processed_para_mono/$SRC-$TGT
...
SRC_RAW=$DATA_PATH/$DATA_NAME-train.tok.$SRC.para_mono
TGT_RAW=$DATA_PATH/$DATA_NAME-train.tok.$TGT.para_mono
SRC_TOK=$SRC_RAW.tok
TGT_TOK=$TGT_RAW.tok
...
if [ "$SRC" == "de" -a "$TGT" == "en" ]; then
  PARA_SRC_TRAIN=$DATA_PATH/$DATA_NAME-train.tok.$SRC
  PARA_TGT_TRAIN=$DATA_PATH/$DATA_NAME-train.tok.$TGT
  PARA_SRC_VALID=$DATA_PATH/$DATA_NAME-dev.tok.$SRC.trim
  PARA_TGT_VALID=$DATA_PATH/$DATA_NAME-dev.tok.$TGT.trim
  PARA_SRC_TEST=$DATA_PATH/$DATA_NAME-test.tok.$SRC.trim
  PARA_TGT_TEST=$DATA_PATH/$DATA_NAME-test.tok.$TGT.trim
fi
...
echo "Tokenizing parallel train, valid and test data..."
...
$FASTBPE applybpe $PARA_SRC_TRAIN_BPE $PARA_SRC_TRAIN.tok $BPE_CODES $SRC_VOCAB
$FASTBPE applybpe $PARA_TGT_TRAIN_BPE $PARA_TGT_TRAIN.tok $BPE_CODES $TGT_VOCAB
