...
epoch_size=$(cat data/$ORDERED_SRC-$ORDERED_TGT/$TGT_DATA_NAME/processed/$ORDERED_SRC-$ORDERED_TGT/train.$SRC | wc -l)
...
python -W ignore train.py \
    --exp_name IBT_BACK_src_$SRC_DATA_NAME\_tgt_$TGT_DATA_NAME\_$SRC\_$TGT \
    --dump_path ./tmp/ \
    --reload_model ${PRETRAINED_MODEL_DIR/mlm_en${OTHER_LANG}_1024.pth,${PRETRAINED_MODEL_DIR/mlm_en${OTHER_LANG}_1024.pth \
    --data_path data/$ORDERED_SRC-$ORDERED_TGT/$TGT_DATA_NAME/processed/$ORDERED_SRC-$ORDERED_TGT \
    --para_data_path data/$ORDERED_SRC-$ORDERED_TGT/$TGT_DATA_NAME/back_translate/$SRC_DATA_NAME \
    --lgs $SRC-$TGT \
    --ae_steps $SRC,$TGT \
