...
epoch_size=$(cat /data/medg/misc/jindi/nlp/datasets/OPUS/$ORDERED_SRC-$ORDERED_TGT/$DATA_NAME/processed/$ORDERED_SRC-$ORDERED_TGT/train.$ORDERED_SRC-$ORDERED_TGT.$SRC | wc -l)
max_epoch_size=300000
epoch_size=$((epoch_size>max_epoch_size ?  max_epoch_size : epoch_size))
echo $epoch_size

python -W ignore train.py \
    --exp_name sup_${DATA_NAME}_subsample_${SAMPLE_SIZE}_$SRC\_$TGT \
    --dump_path ./tmp/ \
    ...
    --data_path /data/medg/misc/jindi/nlp/datasets/OPUS/$ORDERED_SRC-$ORDERED_TGT/$DATA_NAME/processed_subsample_${SAMPLE_SIZE} \
    --lgs $SRC-$TGT \
    --mt_steps $SRC-$TGT \
    --encoder_only false \
    --emb_dim 1024 \
